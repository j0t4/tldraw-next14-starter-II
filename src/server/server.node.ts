...
app.register(async (app: any) => {
	// This is the main entrypoint for the multiplayer sync
	app.get('/connect/:roomId', { websocket: true }, async (socket: any, req: any) => {
	...
	app.put('/uploads/:id', {}, async (req: any, res: any) => {
		const id = (req.params as any).id as string
		await storeAsset(id, req.raw)
		res.send({ ok: true })
	})
	app.get('/uploads/:id', async (req: any, res: any) => {
		const id = (req.params as any).id as string
	...
	app.get('/unfurl', async (req: any, res: any) => {
